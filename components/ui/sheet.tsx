...
const sheetVariants = cva(
  "fixed z-50 flex flex-col gap-4 bg-background p-6 shadow-[var(--shadow-floating)] data-open:animate-in data-closed:animate-out data-open:duration-300 data-closed:duration-200 data-open:ease-[cubic-bezier(0.22,1,0.36,1)]",
  {
    variants: {
      side: {
        right:
  ...
  return (
    <SheetPortal>
      ...
      >
        {children}
        {showCloseButton && (
          <SheetPrimitive.Close className="absolute top-4 right-4 rounded-md p-1.5 text-muted-foreground opacity-80 transition hover:bg-accent hover:opacity-100 focus-visible:ring-3 focus-visible:ring-ring/50 focus-visible:outline-none">
